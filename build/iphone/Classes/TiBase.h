...
// in simulator we redefine to format for WXJM Developer console
...
{\
	NSString * exceptionName = [@"org.wxjm." stringByAppendingString:NSStringFromClass([self class])];\
...
+ (void) throwException:(NSString *) reason subreason:(NSString*)subreason location:(NSString *)location\
{\
	NSString * exceptionName = @"org.wxjm";\
	NSString * message = [NSString stringWithFormat:@"%@. %@ %@",reason,(subreason!=nil?subreason:@""),(location!=nil?location:@"")];\
	NSLog(@"[ERROR] %@",message);\
	if ([NSThread isMainThread]==NO) {\
		@throw [NSException exceptionWithName:exceptionName reason:message userInfo:nil];\
...
enum {
	UIWXJMNativeItemNone = -1, 
	UIWXJMNativeItemSpinner = -2,
	UIWXJMNativeItemProgressBar = -3,
	
	UIWXJMNativeItemSlider = -4,
	UIWXJMNativeItemSwitch = -5,
	UIWXJMNativeItemMultiButton = -6,
	UIWXJMNativeItemSegmented = -7,
	
	UIWXJMNativeItemTextView = -8,
	UIWXJMNativeItemTextField = -9,
	UIWXJMNativeItemSearchBar = -10,
	
	UIWXJMNativeItemPicker = -11,
	UIWXJMNativeItemDatePicker = -12,
	
	UIWXJMNativeItemInfoLight = -13,
	UIWXJMNativeItemInfoDark = -14,
	
	UIWXJMNativeItemDisclosure = -15,
	
	UIWXJMNativeItemContactAdd = -16
};
